...
from Liquirizia.WebApplication import Response as Response
...
class ResponseWriter(MutableMapping, metaclass=Meta):
	...
	def __init__(self, response: Response):
		...
		self.properties = {
			...
			'headers': {ToHeaderName(header[0]): [{ToHeaderName(header[0]): header[1]}] for header in response.headers()},
			'body': response.body if response.body else None
		}
		pass
