...
이벤트 어플리케이션(EventRunner)을 이벤트 스케줄러(EventScheduler)형식의 아마존 웹 서비스 람다 함수로 사용하기 위한 핸들러
...
properties = EventRunnerProperties(
	SampleEventRunner,
	type=EventTimer('event.sample.compute', '*/10 * * * * *', headers={'a': 6, 'b': 2}, body={'c': 3}),
	header=Validator(
...
### 함수 이벤트 핸들러 정의
```python
from Liquirizia.AWS.Function.Implements.EventScheduler import FunctionEventHandler
